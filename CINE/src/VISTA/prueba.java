...
public class prueba extends JPanel {
	 ...
	 * Create the panel.
	 */
	public prueba(pelicula[] peliculas,String nombrePelicula,String tipo,cine ARCANE_,int nAsientos_, ArrayList<String> nombres_,
        ...
        for(i=0; i<10;i++) {
			...
			public void actionPerformed(ActionEvent e) {
				if(estoyEnArray(auxButton.getText().toString())&&(ln_pA.getNombresI().size()+ln_pA.getNombresN().size())<nAsientos) {
        ...
        for(i=0; i<5;i++) {
        	for(int j=0; j<10;j++) {
				...
				public void actionPerformed(ActionEvent e) {
					if(estoyEnArray(auxButton2.getText().toString())&&(ln_pA.getNombresI().size()+ln_pA.getNombresN().size())<nAsientos) {						
	...
	public boolean estoyEnArray(String nombreBotones) {
		boolean flag=true;
		...
		for(String auxNombre1:ln_pA.getNombresN()) {
			if(auxNombre1.contains(nombreBotones)) {
				flag=false;
			}
		}
		return flag;
